...
1. `/contact/courses` → `course_inquiries`. `parseDateRange.ts` extracts a date window from free-text ("sometime mid-June?"); `inquiryConflicts.ts` finds overlapping requests and suggests grouping students into one course. `extractInquiryFacts.ts` runs an LLM pass at insert time (via `after()`, zero route latency) filling `parsed_headcount`, `date_flexibility`, `ai_facts`, and — only where the regex left them NULL — `parsed_start_date`/`parsed_end_date`; columns in `supabase/inquiry-intel.sql`. Backfill/status: `GET /api/admin/inquiries/intel?run=8` (admin auth); the digest cron sweeps 3 unprocessed rows per run as the catch-up.
   The intelligence layer on top: `schedule.ts` (live courses + seats left from bookings, personal busy via `PERSONAL_ICS_URLS` ICS feeds, open weekends), `demandClusters.ts` (headcount-weighted clusters matched to scheduled events / open weekends, lapsed-window triage, duplicate emails), `inquiryReply.ts` (the drafting/sending engine — Joshua's voice + AIDA standards + live schedule context; shared by the reply route and `/admin/act`). The digest embeds signed one-tap links (`actionTokens.ts`, HMAC over ADMIN_KEY, 14-day expiry): draft/send a reply, mark replied, archive — GET only renders, all mutations are POSTs, so email prefetchers are inert.
   `gmailSync.ts` reconciles the pipeline with the business Gmail over IMAP (`GMAIL_USER` + `GMAIL_APP_PASSWORD` app password — deliberately not OAuth: Gmail scopes are restricted, and personal-account refresh tokens expire weekly in testing mode). Targeted searches only (per active inquiry address, never a mailbox scan), columns in `supabase/email-sync.sql`; the ONE status it ever changes is new → replied when Joshua's outbound postdates the inquiry. Runs inside the digest; on-demand via `GET /api/admin/email-sync?run=true&days=N`.
   `stripeSync.ts` reconciles payments against the VIRIDITAS LLC Stripe account (`STRIPE_VIRIDITAS_READ_KEY`, a restricted read-only key — deposit invoices are issued from Viriditas, so the site's own Stripe webhook never sees them). Per active inquiry it searches succeeded charges + paid invoices under the student's email; because Viriditas bills other ventures, only COURSE-looking payments (aida/course/freedive/coaching/deposit descriptions) advance the ONE status it ever changes: active → paid (amount+date appended to admin_notes). Zelle/bank transfers stay manual. Runs inside the digest; on-demand via `GET /api/admin/stripe-sync?run=true&days=N`; also in Telegram `/sync`.
   Mailbox routing: `BUSINESS_EMAIL` (lajollafreediveclub@gmail.com) receives all form/payment notifications (course-inquiry, waiver, camp, AIDA forms, Saturday, Stripe) and is the reply-to on student-facing mail; the public site displays it. `GMAIL_ALIASES` covers "Send mail as" addresses. Cron reports (daily-email, course-briefing, friday-reminder) still go to the personal address; the digest goes to both.
   Rosters: `/admin/roster` (page) + `/api/admin/roster` — attaching an inquiry to a course event creates the bookings row that seat counts everywhere derive from (homepage strip, /programs, /calendar, digest, bot all read calendar_events + bookings via getScheduledCourses). Unroster refuses Stripe-checkout bookings; "complete" (only after the course date passes) flips all rostered students' inquiries to completed. The digest banners courses that ran in the last 3 days with un-completed rosters. Event `spots` text = TOTAL capacity — never hand-decrement it; enrollment is bookings' job.
   Gmail reading: `gmailRead.ts` + `GET /api/admin/inbox?email=X&days=N` — fetches actual message BODIES for one address over IMAP (targeted searches, first text/plain part, reply-tails trimmed, truncated) so the agent reads student threads without a browser. Gmail drafts: `gmailDrafts.ts` creates real Gmail drafts over IMAP APPEND (same app password as the sync — no OAuth, no new creds; Gmail does NOT auto-append the account signature to appended drafts, so bodies carry the sign-off). Used by Telegram `/draft` (draft lands in Gmail alongside the one-tap link) and blast POST `mode: "gmail_draft"` (drafts instead of sends; no status change — Joshua's own send from Gmail is what the next gmailSync picks up as outbound).
   Confirmation blast: `/admin/blast` (page) + `/api/admin/blast` — GET matches every active inquiry to a published course date (AIDA 1/2 keyword + window overlap; full courses count as "held seats" for quoted people) and drafts personalized confirmations in chunks of 8; POST sends reviewed/edited drafts (cap 30), then restores/advances status (new/replied → quoted, others keep theirs — `sendInquiryReply` stomps to replied first) and stamps admin_notes. The review tap is the authorization; the API never sends what wasn't posted to it. The homepage `UpcomingCourses` strip (ISR 300s) renders the same `getScheduledCourses` truth.
   Telegram agent: webhook at `POST /api/telegram` (secret_token derived from ADMIN_KEY; register via admin `GET /api/telegram?setup=true`), pinned to `TELEGRAM_CHAT_ID`. Commands /pipeline /clusters /schedule /sync /draft /act; free text answered by Claude grounded in a live pipeline+schedule snapshot (`telegramBot.ts`). The bot never emails students — /draft hands back a signed /admin/act link and the tap is the send.
